...
INSERT INTO `jl_news` (`news_id`, `news_name`, `news_link`, `news_description`, `news_time`, `news_content`) VALUES
...
CREATE TABLE IF NOT EXISTS `jl_products` (
  ...
  `products_content` text NOT NULL COMMENT '产品信息内容',
...
INSERT INTO `jl_products` (`products_id`, `products_name`, `products_img`, `products_content`) VALUES
(22, 'Jeffery', '1-140923112141W4.jpg', '的撒旦撒旦'),
(21, 'Jeffery112313', '1-140R1093501M6.gif', '下午帮我第三，打开'),
(20, 'Jeffery', '1-140Q41646460-L.gif', '打算的撒旦的撒旦撒旦');
...
CREATE TABLE IF NOT EXISTS `ty_admin` (
  `admin_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '管理员ID',
  `admin_username` varchar(100) NOT NULL COMMENT '管理员账户',
  `admin_password` varchar(100) NOT NULL COMMENT '管理员密码',
  `admin_email` varchar(100) NOT NULL COMMENT '管理员电子邮件',
  PRIMARY KEY (`admin_id`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 AUTO_INCREMENT=2 ;
...
INSERT INTO `ty_admin` (`admin_id`, `admin_username`, `admin_password`, `admin_email`) VALUES
(1, '123', '1234', '13@qq.com');
...
CREATE TABLE IF NOT EXISTS `ty_nav` (
  `nav_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '导航ID',
  `nav_name` varchar(100) NOT NULL COMMENT '导航名称',
  `nav_link` varchar(100) NOT NULL COMMENT '导航链接',
  `nav_order` int(10) unsigned NOT NULL COMMENT '导航排序',
  PRIMARY KEY (`nav_id`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 AUTO_INCREMENT=8 ;
...
INSERT INTO `ty_nav` (`nav_id`, `nav_name`, `nav_link`, `nav_order`) VALUES
(1, '网站首页', 'index.php', 60),
(2, '服务体系', 'server.php', 50),
(3, '公司新闻', 'news.php', 40),
(4, '案例展示', 'case.php', 10),
(5, '在线留言', 'meg.php', 0),
(6, '关于泰一', 'contact.php', 0);
...
CREATE TABLE IF NOT EXISTS `ty_news` (
  `news_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '新闻编号',
  `news_name` tinytext NOT NULL COMMENT '新闻标题',
  `news_content` text NOT NULL COMMENT '新闻内容',
  `news_time` int(10) unsigned NOT NULL COMMENT '发布时间',
  `news_small_thumb` tinytext NOT NULL COMMENT '新闻小的缩略图',
  `news_big_thumb` tinytext NOT NULL COMMENT '新闻大缩略图(新闻内容中)',
  `news_links` varchar(100) NOT NULL COMMENT '新闻链接',
  `news_description` text NOT NULL COMMENT '新闻描述',
  PRIMARY KEY (`news_id`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 AUTO_INCREMENT=2 ;
...
INSERT INTO `ty_news` (`news_id`, `news_name`, `news_content`, `news_time`, `news_small_thumb`, `news_big_thumb`, `news_links`, `news_description`) VALUES
(1, '2008年,广东省科技报指定网络运营商以及合作伙伴', '2011.5.13 《建设领域应用软件测评通用规范》修订编制组成立会于北京召开。丝路数码科技有限公司作为15家编委会成员之一，集团总裁李萌迪，北京丝路总经理李鹏辉应邀出席会议2011.5.13 《建设领域应用软件测评通用规范》修订编制组成立会于北京召开。丝路数码科技有限公司作为15家编委会成员之一，集团总裁李萌迪，北京丝路总经理李鹏辉应邀出席会议', 1443052800, '', '', '', '年获批省级项目《岭南建筑文化》三维动画制作以及策划推广2010年，执行广东产学院五周年庆典...');
